...
def mouse_callback(event, x, y, flags, param):
    if event == cv2.EVENT_LBUTTONDOWN:
        ...
        print(f"坐标点({x}, {y})的深度值为: {depth_value:.3f}米")
...
def demo_one_sample(model, model_name, device, sample, cano_sz, args: argparse.Namespace):
    ...
    pred_depth_full = cv2.resize(pred_depth, (1702, 1276), interpolation=cv2.INTER_LINEAR) #恢复大小
    ...
    callback_params = {'depth_map': pred_depth_full, 'last_click': None}
    cv2.setMouseCallback('Depth Map', mouse_callback, 
                        {'depth_map': pred_depth_full, 'last_click': None})
    ...
    #交互
    while True:
        display_img = depth_vis_color.copy()
        # 获取鼠标回调中保存的最后点击位置
        last_click = callback_params['last_click']
        if last_click is not None:
            x, y = last_click
            depth_value = pred_depth_full[y, x]
            text = f"深度: {depth_value:.3f}m"
        ...
        cv2.imshow('Depth Map', resized_img)
