...
def mouse_callback(event, x, y, flags, param):
    if event == cv2.EVENT_LBUTTONDOWN:
        ...
        if 0 <= x_original < pred_depth_map.shape[1] and 0 <= y_original < pred_depth_map.shape[0]:
            ...
            print(f"坐标点({x_original}, {y_original})的预测深度值为: {pred_depth_value:.3f}米, 真实深度值为: {gt_depth_value:.3f}米")
...
def demo_one_sample(model, model_name, device, sample, cano_sz, args: argparse.Namespace):
    ...
    if sample["annotation_filename_depth"] is None:
        ...
        #     / sample["depth_scale"]
        # )
        depth = cv2.imread(sample["annotation_filename_depth"], cv2.IMREAD_ANYDEPTH)
    ...
    callback_params = {
        'pred_depth_map': pred_depth_full,  # 模型预测的深度图
        'gt_depth_map': depth,              # 真实深度图
        'last_click': None,
    ...
    while True:
        ...
        if last_click is not None:
            x, y = last_click
            pred_depth_value = pred_depth_full[y, x] # 获取预测深度值
            gt_depth_value = depth[y, x]              # 真实深度值
            text = f"预测深度: {pred_depth_value:.3f}m, 真实深度: {gt_depth_value:.3f}m"
            cv2.circle(display_img, (x, y), 5, (0, 255, 255), -1)
            cv2.putText(display_img, text, (x+10, y-10), font, 0.6, (255, 255, 255), 2)
        # resized_img = cv2.resize(display_img, (0, 0), fx=0.5, fy=0.5)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--sample-file", type=str, default='data/nyu.json')
