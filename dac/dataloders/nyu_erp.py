...
class NYUERPDataset(BaseDataset):
    ...
    def __getitem__(self, idx):
        ...
        # Add camera model parameter to nyu_cam_params
        self.nyu_cam_params['camera_model'] = 'PINHOLE'
        ...
        assert isinstance(gts["gt"], torch.Tensor), "GT should be Tensor"
        ...
        if self.test_mode:
            return {
                ...
                "lat_range": torch.from_numpy(lat_range).float(),
                "long_range": torch.from_numpy(long_range).float(),
                "info": info
            }
        else:
            return {
