...
class RunningMetric(object):
    def __init__(self, metrics_names: List[str]):
        super().__init__()
        self.metrics_dict = {k: torch.tensor(0.0) for k in metrics_names}
        self.count = 0
    ...
    def accumulate_metrics(self, gt, pred, mask=None):
        ...
        for k, v in self.metrics_dict.items():
            self.metrics_dict[k] = globals()[k](
                gt, pred, v, self.count, new_samples, splits
            )
        self.count += new_samples

    def update(self, metrics_dict: dict):
        for k, v in metrics_dict.items():
            if not isinstance(v, torch.Tensor):
                v = torch.tensor(v, device=self.metrics_dict[k].device)
            self.metrics_dict[k] += v.detach()
        self.count += 1
    ...
    def reset_metrics(self):
        for k, v in self.metrics_dict.items():
            self.metrics_dict[k] = torch.tensor(0.0)
        self.count = 0
